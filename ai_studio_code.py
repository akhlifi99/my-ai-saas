...
<style>
    ...
    /* ---------------------------------------------------------------------
       CHAT CONTRAST FIXES
       ---------------------------------------------------------------------
       Make all chat messages, nested paragraphs, markdown, spans, list items,
       and code blocks bright/legible against the dark background.
    */
    ...
    /* Extra safeguard for standard text inputs */
    .stTextInput input,
    .stTextArea textarea {
        color: #ffffff !important;
        caret-color: #ffffff !important;
...
def clear_auth_state() -> None:
    # Keep sidebar state simple; no widget-linked mutations here.
...
def enforce_page_access(user: Dict[str, Any]) -> None:
    ...
    if st.session_state.sidebar_page not in allowed_pages:
        st.session_state.sidebar_page = st.session_state.nav_page
...
def sync_nav_page_from_sidebar() -> None:
    selected = st.session_state.get("sidebar_page", DEFAULT_PAGE)
...
def render_sidebar(user: Dict[str, Any]) -> None:
    with st.sidebar:
        ...
        # Keep widget state in sync BEFORE the radio is instantiated.
        if st.session_state.nav_page not in allowed_pages:
            st.session_state.nav_page = DEFAULT_PAGE
        if st.session_state.sidebar_page not in allowed_pages:
            st.session_state.sidebar_page = st.session_state.nav_page
        if st.session_state.sidebar_page != st.session_state.nav_page:
            st.session_state.sidebar_page = st.session_state.nav_page
        ...
        st.radio(
            ...
            key="sidebar_page",
            on_change=sync_nav_page_from_sidebar,
        )
